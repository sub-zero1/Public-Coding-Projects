...
export const ChannelListItem = ({
	...
	const isOneOnOneConversation = isDirectMessagingConversation && Object.keys(channel.state.members).length === 2;
	...
	if (isOneOnOneConversation) {
		...
		ChannelTitle = (
			<SCText style={channelTitleStyle}>{truncate(getChannelDisplayName(channel, false, true), 40)}</SCText>
		);
		...
		ChannelPrefix = (
			<SCText style={styles.directMessagingConversationPrefix}>{channel.data.member_count - 1}</SCText>
		);
		ChannelTitle = <SCText style={channelTitleStyle}>{truncate(getChannelDisplayName(channel), 40)}</SCText>;
	} else {
		...
		<TouchableOpacity
			...
			{isDirectMessagingConversation && isUnread && (
				<View style={styles.unreadMentionsContainer}>
					<SCText style={styles.unreadMentionsText}>{channel.countUnread()}</SCText>
				</View>
			)}
			{(!isDirectMessagingConversation && countUnreadMentions) > 0 && (
				<View style={styles.unreadMentionsContainer}>
					<SCText style={styles.unreadMentionsText}>{countUnreadMentions}</SCText>
				</View>
			)}
		</TouchableOpacity>
	);
			...
			style={
				online
					...
					: [
							...
							{
								backgroundColor: backgroundTransparent ? 'transparent' : colors.background,
