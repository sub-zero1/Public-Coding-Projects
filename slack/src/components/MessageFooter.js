...
export const MessageFooter = (props) => {
	...
	return (
		<View style={styles.reactionListContainer}>
			...
			{props.message.latest_reactions && props.message.latest_reactions.length > 0 && (
				<TouchableOpacity
...
export const renderReactions = (reactions, ownReactions = [], supportedReactions, reactionCounts, handleReaction) => {
...
const ReactionItem = ({ type, handleReaction, reactionCounts, emojiDataByType, ownReactionTypes }) => {
	const { dark } = useTheme();
